...
import { Check, AlertTriangle, FileText, ClipboardCheck } from "lucide-react";
...
export function CountryCard({ country }: CountryCardProps) {
  ...
  const handleCheckRequirementsClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    setLocation(`/assess?destination=${country.code}`);
  };
      ...
      <CardContent className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between gap-2 text-sm flex-wrap">
          ...
          <Button 
            ...
            onClick={handleCheckRequirementsClick}
            data-testid={`button-check-requirements-${country.id}`}
          >
